...
<ul class="products-grid products-grid--max-4-col first last odd">
  ...
  >
    ...
    <div class="product-info" style="min-height: 99px">
      ...
      <div @submit.prevent="sepeteekle" class="actions" style="display: block">
        <button 
          type="button"
          title="HEMEN AL"
...
export default {
  data(){
    ...
    return{products:[],inCart:[],sepetUrunn:''}
  },
  mounted(){
  ...
  sepeteekle(){
    const sepetUrun=this.inCart.slice();
    sepetUrun.push(this.sepetUrun);
    fbDb.ref('inCart').child('adet').set(sepetUrunn);
    fbDb.ref('inCart').child('id').set(sepetUrunn);
    fbDb.ref('inCart').child('image').set(sepetUrunn);
    fbDb.ref('inCart').child('urunFiyat').set(sepetUrunn);
    fbDb.ref('inCart').child('urunadi').set(sepetUrunn);
    fbDb.ref('inCart').child('uruncode').set(sepetUrunn);
  }
