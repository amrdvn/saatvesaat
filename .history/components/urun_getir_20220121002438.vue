...
<ul class="products-grid products-grid--max-4-col first last odd">
  ...
  >
    ...
    <a 
      ...
      class="product-image"
    >
      <span v-model="sepeteEkle" data-pid="56675" class="link-wishlist addToWishlist"></span>
      <img 
        class="lazy-loaded"
...
export default {
  data(){
    ...
    return{products:[],musteri:[],sepeteEkle:''}
  },
  mounted(){
  ...
  sepeteEkle(){
    const yeni=this.musteri.slice();
    yeni.push(sepeteEkle);
    fbDb.ref('Musteri').child('musteriMailAdresi').set(yeni);
    fbDb.ref('Musteri').child('urunKodu').set(yeni);
