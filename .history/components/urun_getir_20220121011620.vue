  ...
  >
    ...
    <div class="product-info" style="min-height: 99px">
      <input 
        ...
        :value="product.ad"
      />
      <span itemprop="brand" class="brand">{{urunMarkaEkle=product.marka}}{{product.marka}}</span>
      <span itemprop="name" class="product-name center-block"
        ><span :title="product.ad">
          {{urunAdEkle=product.ad}}{{product.ad}}</span
        >
      </span>
      ...
      >
        ...
        <span class="regular-price ln189" id="product-price-56675">
          <span class="price">{{urunFiyatEkle=product.fiyat}}{{product.fiyat}} TL&nbsp;</span>
        </span>
      </div>
...
export default {
  data(){
    ...
    return{products:[],musteri:[{
    urunKoduEkle:'asdf',
    urunAdetEkle:'df',
    urunFiyatEkle:'adsf',
    urunResimUrlEkle:'adsf',
    urunAdEkle:'adfs',
    urunMarkaEkle:'afdsasdf',}],
   // mailEkle:'',
    
    }
  },
  mounted(){
    fbDb.ref('products').get().then((snapshot)=>{
      if(snapshot.exists()){
  ...
  sepeteEkle(){
    // const mail=this.musteri.slice();
    // mail.push(this.mailEkle);
    // fbDb.ref('Musteri').child('musteriMailAdresi').set(mail);
    const kod=this.musteri.slice();
    kod.push(this.urunKoduEkle);
    fbDb.ref('Musteri').child('urunKodu').set(kod);
     const adet=this.musteri.slice();
    adet.push(this.urunAdetEkle);
    fbDb.ref('Musteri').child('urunAdet').set(adet);
     const fiyat=this.musteri.slice();
    fiyat.push(this.urunFiyatEkle);
    fbDb.ref('Musteri').child('urunFiyat').set(fiyat);
     const resim=this.musteri.slice();
    resim.push(this.urunResimUrlEkle);
    fbDb.ref('Musteri').child('urunResim_url').set(resim);
     const ad=this.musteri.slice();
    ad.push(this.urunAdEkle);
    fbDb.ref('Musteri').child('urunAd').set(ad);
     const marka=this.musteri.slice();
    marka.push(this.urunMarkaEkle);
    fbDb.ref('Musteri').child('urunMarka').set(marka);
  }
