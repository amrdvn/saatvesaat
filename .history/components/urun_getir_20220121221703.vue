<template>
  <ul class="products-grid products-grid--max-4-col first last odd">
    ...
    >
      ...
      ><my-component v-for="(id,urunCode,urunAdet) in sepet" v-bind:id="product.fiyat" v-bind:urunAdet="1" v-bind:urunCode="asdf"></my-component>
        <tbody>
          <tr>
...
export default {
  ...
  mounted() {
    ...
    fbDb.ref('sepet').get().then((snapshot)=>{if(snapshot.exists()){this.sepet=snapshot.val()}})
  },
