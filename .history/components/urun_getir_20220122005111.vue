...
export default {
  ...
  mounted() {
    ...
    fbDb.ref('sepet').get().then((snapshot)=>{
      if(snapshot.exists()){
        this.sepet.id=this.products.id
        this.sepet.urunAdet='2'
        this.sepet.urunCode=this.products.code
        }})
  },
  ...
  methods: {
    sepeteEkle() {
      ...
      fbDb.ref('Musteri/'+ this.user.uid).set(this.productsZZZS)
    },
  },
}
</script>
