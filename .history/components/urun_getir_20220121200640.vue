  ...
  <ul class="products-grid products-grid--max-4-col first last odd">
    ...
    >
      ...
      >
        <tbody>
          <tr>
            <td style="background: url() no-repeat 0 0">
...
export default {
  data() {
    return { products: [], user: '',sepet:[{urunFiyat: [],
  urunResimUrlEkle: [],
  urunAdEkle: [],
  urunMarkaEkle: [],
  urunCinsiyet: [],
    ...
    fbDb
      ...
      .then((snapshot) => {
        if (snapshot.exists()) {
          ...
          this.urunAdEkle = snapshot.val()
          this.urunCinsiyet = snapshot.val()
          this.urunCode = snapshot.val()
          this.urunFiyat = snapshot.val()
          this.urunId = snapshot.val()
          this.urunResimUrlEkle = snapshot.val()
          this.urunMarkaEkle = snapshot.val()
          this.urunOzellik = snapshot.val()
        }
      })
  ...
  methods: {
    sepeteEkle() {
      fbDb.ref('Musteri/' + this.user.uid).set(sepet)
    },
  },
}
</script>
...
@media (min-width: 1200px) .amlabel-div {
  width: 100%;
  display: inline-block;
  position: relative;
