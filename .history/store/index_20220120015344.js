...
import { realDb, auth } from '../plugins/firebase';
...
export const actions = {
  ...
  addToCart({ commit, dispatch, state }, payload) {  // ürünü sepete ekler
    onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        if (finditem) {
          const reference = ref(realDb, "musteri/" * user.uid + "/musteri/" + finditem.key + "/adet")
  ...
  removeFromCart({ commit, state, dispatch }, id) { // ürünü sepetten çıkarır
    onAuthStateChanged(auth, (user) => {
      if (user) {
        const cartref = ref(realDb, "musteri/" + user.uid + "/musteri")
  ...
  register({ commit }, payload) {
    createUserWithEmailAndPassword(auth, payload.email, payload.password)
      .then((userCredential) => {
        ...
        const userref = ref(realDb, "musteri/" + newUser.uid)
        const userData = {
          adresses: null,
          birthDay: "01-01-1998",
          couponsId: null,
