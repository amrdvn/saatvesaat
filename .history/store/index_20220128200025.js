...
export const state = () => ({
  ...
  export const actions = {
    ...
    sepeteEkle({ commit, dispatch, state }, payload) {  // ürünü sepete ekler
        firebase.auth().onAuthStateChanged((user) => {
        const finditem = state.inCart.find(item => item.id === payload.id)
        if (user) {
          if (finditem) {
            const reference = ref(fbDb, "Sepet/" * user.uid + "/adet")
            set(reference, finditem.adet + 1)
          }
          else {
            const cartref = ref(fbDb, "Sepet/" + user.uid + "/adet") // yolu(link)
    ...
    sepettenCikar({ commit, state, dispatch }, code) { // ürünü sepetten çıkarır
        firebase.auth().onAuthStateChanged((user) => {
        if (user) {
          const cartref = ref(fbDb, "Sepet/" + user.uid )
          const key = state.inCart.find(item => item.code === secode).key // dictionary içerisindeki bulunan bu key'e
