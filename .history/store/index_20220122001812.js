...
import realDBRefRefRef from '../plugins/firebase.js';
...
export const actions = {
  fetchproducts({ commit, dispatch }) {
    const realDBRefRef = ref(realDBRef, 'products');
    onValue(realDBRefRef, (snapshot) => {
  ...
  fetchCart({ commit, dispatch }) { // sepetime eklenen ürünlerin verilerini sepetimde görüntülenmesini sağlıyor.
    onAuthStateChanged(auth, (user) => {
      let arr = []
      if (user) {
        const cartref = ref(realDBRef, "users/" + user.uid + "/inCart")
  ...
  addToCart({ commit, dispatch, state }, payload) {  // ürünü sepete ekler
    onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        if (finditem) {
          const reference = ref(realDBRef, "users/" * user.uid + "/inCart/" + finditem.key + "/count")
          set(reference, finditem.count + 1)
        }
        else {
          const cartref = ref(realDBRef, "users/" + user.uid + "/inCart") // yolu(link)
  ...
  register({ commit }, payload) {
    createUserWithEmailAndPassword(auth, payload.email, payload.password)
      .then((userCredential) => {
        ...
        const userref = ref(realDBRef, "users/" + newUser.uid)
        const userData = {
          adresses: null,
          birthDay: "01-01-1998",
          couponsId: null,
